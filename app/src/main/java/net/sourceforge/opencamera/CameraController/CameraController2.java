...
//		Frame duration is in nanoseconds.  Using min to be safe.
		...
		try {
			...
			class MyStateCallback extends CameraCaptureSession.StateCallback {
				...
				public void onConfigured(@NonNull CameraCaptureSession session) {
					if( MyDebug.LOG ) {
						Log.d("StateCallback", "onConfigured: " + session);
				...
				public void onConfigureFailed(@NonNull CameraCaptureSession session) {
					if( MyDebug.LOG ) {
						Log.d("StateCallback", "onConfigureFailed: " + session);
						Log.d(TAG, "captureSession was: " + captureSession);
					}
				    synchronized( create_capture_session_lock ) {
				    	callback_done = true;
				...
				public void onCaptureQueueEmpty(CameraCaptureSession session) {
					if (MyDebug.LOG)
						Log.d("StateCallback", "onCaptureQueueEmpty: " + session);
				}

				@Override
				public void onReady(CameraCaptureSession session) {
					if (MyDebug.LOG)
						Log.d("StateCallback", "onReady: " + session);
				}

				@Override
				public void onClosed(CameraCaptureSession session) {
					if (MyDebug.LOG)
						Log.d("StateCallback", "onClosed: " + session);
				}

				@Override
				public void onActive(CameraCaptureSession session) {
					if (MyDebug.LOG)
						Log.d("StateCallback", "onActive: " + session);
				}
	...
	public void stopBurst() {
		...
		try {
			if (captureSession != null) {
				Log.d(TAG, "stopBurst n_remaining: " + n_burst);
				//captureSession.stopRepeating();
				captureSession.abortCaptures();
			}
		} catch (CameraAccessException e) {
			Log.d(TAG, "finishBurst : Failed by CameraAccessException", e);
		} catch (IllegalStateException e) {
			Log.d(TAG, "finishBurst : Failed by IllegalStateException", e);
		} finally {
			PictureCallback cb = jpeg_cb;
			jpeg_cb = null;
			if( raw_cb == null ) {
